...
graph_t create_test_graph(int c, int h, int w)
{
    ...
    if(create_conv_node(graph, conv_name, input_name, 3, 1, 1, c, out_c, 1) < 0)
    {
...
int main(int argc, char* argv[])
{
    ...
    if(new_graph==NULL)
    {
        printf("unpack graph failed\n");

        return 1;
    }

    context_t new_context=create_context("test",0);
    new_graph->exec_attr->exec_context=new_context;
