...
static int load_op_move(struct ir_node* ir_node, struct tiny_node* tiny_node)
{
    struct tiny_move_param* tiny_param = ( struct tiny_move_param* )tiny_node->op_param;
    struct mv_param* mv_param = ( struct mv_param* )(ir_node->op.param_mem);

    mv_param->start_mv_addr = tiny_param->start_mv_addr;
    mv_param->mv_size = tiny_param->keep_size;
    mv_param->buffer_size = tiny_param->buffer_size;
    mv_param->current_buffer_size = tiny_param->current_buf_size;
    mv_param->tmp_buffer_out_size = tiny_param->temp_out_size;
	mv_param->flag = tiny_param->flag ;
...
static int init_tiny_serializer(struct serializer* s)
{
	...
	op_loader_map[NN_OP_MOVE] = load_op_move;
