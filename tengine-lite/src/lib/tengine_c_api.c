...
#define TENGINE_LITE_VERSION "0.1"
...
tensor_t DLLEXPORT get_graph_tensor(graph_t graph, const char* tensor_name)
{
    ...
    int idx = get_node_idx_from_name(ir_graph, tensor_name);
...
int DLLEXPORT prerun_graph_multithread(graph_t graph, int num_thread)
{
    struct ir_graph* ir_graph = ( struct ir_graph* )graph;
    ...
    if(infer_shape_graph(ir_graph) < 0)
    {
        ...
        printf("infer_shape_graph failed\n");
        return -1;
    }
    ...
    {
        ir_graph->status = GRAPH_STAT_ERROR;
        printf("allocator->allocate failed\n");
        return -1;
    }

    struct exec_scheduler* scheduler = context->scheduler;

    if(scheduler->prerun(scheduler, ir_graph, num_thread) < 0)
    {
        ir_graph->status = GRAPH_STAT_ERROR;
        printf("scheduler->prerun failed\n");
        return -1;
    }
...
int DLLEXPORT prerun_graph(graph_t graph)
{
    ...
    if(scheduler->prerun(scheduler, ir_graph, 1) < 0)
    {
        ir_graph->status = GRAPH_STAT_ERROR;
        return -1;
    }
