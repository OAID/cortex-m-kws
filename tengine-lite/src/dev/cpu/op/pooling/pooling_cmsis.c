...
void arm_maxpool_HWC_q7_nonsquare(q7_t* Im_in, const uint16_t dim_im_in_x, const uint16_t dim_im_in_y,
...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    arm_maxpool_HWC_q7_nonsquare(input_tensor->data, input_tensor->dims[2], input_tensor->dims[1],
...
static int reg_pooling_cmsis_ops(void* arg)
{
    return register_builtin_node_ops(OP_POOL, &cmsis_node_ops);
}

static int unreg_pooling_cmsis_ops(void* arg)
{
