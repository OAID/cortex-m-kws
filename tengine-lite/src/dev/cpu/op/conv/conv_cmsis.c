...
static int reg_conv_cmsis_ops(void* arg)
{
    return register_builtin_node_ops(OP_CONV, &cmsis_node_ops);
}

static int unreg_conv_cmsis_ops(void* arg)
{
    unregister_builtin_node_ops(OP_CONV, &cmsis_node_ops);
    return 0;
}

AUTO_REGISTER_OPS(reg_conv_cmsis_ops);
AUTO_UNREGISTER_OPS(unreg_conv_cmsis_ops);
